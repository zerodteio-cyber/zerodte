...
function score(levels: typeof defaultLevels) {
  let s = 50;
  if (levels.spyPrice > levels.kingNode) s += 15;
  else s -= 15;
  if (levels.spyPrice > levels.hvl) s += 10;
  else s -= 10;
  if (levels.vix < 15) s += 10;
  else if (levels.vix > 20) s -= 15;
  else s -= 5;
  if (levels.spyPrice > levels.putWall && levels.spyPrice < levels.callWall) s += 10;
...
export default function Dashboard() {
  ...
  const bias = s >= 65 ? "CALLS" : s >= 45 ? "NO TRADE — WAIT FOR CLARITY" : "PUTS";
    ...
    <main style={{ minHeight: "100vh", background: "#0a0a0a", color: "#fff", fontFamily: "Courier New, monospace" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 40px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 40 }}>
          <div>
            <p style={{ color: "#444", fontSize: 11, letterSpacing: 3, marginBottom: 6 }}>{levels.date}</p>
          ...
          <button onClick={() => { setEditing(!editing); setDraft(levels); }} style={{ background: editing ? "#333" : "#111", color: editing ? "#fff" : "#555", border: "1px solid #222", padding: "10px 20px", fontSize: 11, letterSpacing: 2, cursor: "pointer", fontFamily: "Courier New" }}>
            {editing ? "CANCEL" : "UPDATE LEVELS"}
          </button>
        </div>

        {editing && (
          <div style={{ background: "#0f0f0f", border: "1px solid #222", padding: 24, marginBottom: 32 }}>
            <p style={{ color: "#555", fontSize: 11, letterSpacing: 3, marginBottom: 20 }}>ENTER TODAY'S GEX LEVELS</p>
            {(["spyPrice", "kingNode", "callWall", "putWall", "hvl", "vix"] as const).map(key => (
              <div key={key} style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 12 }}>
                <label style={{ fontSize: 11, letterSpacing: 2, color: "#555", width: 100, textTransform: "uppercase" }}>{key}</label>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          <div style={{ background: "#0f0f0f", border: "1px solid #1a1a1a", padding: 24 }}>
            ...
            <Level label="VIX" value={levels.vix} prefix="" />
            <Level label="SPY PRICE" value={levels.spyPrice} highlight />
          </div>
          ...
          <div>
            ...
            <div style={{ background: "#0f0f0f", border: `1px solid ${biasColor}33`, padding: 24 }}>
              ...
              <p style={{ fontSize: 11, color: "#333", marginTop: 12, lineHeight: 1.6 }}>
                {levels.spyPrice > levels.kingNode ? "Price above King Node — bulls in control." : "Price below King Node — bears in control."}{" "}
                VIX at {levels.vix} — {levels.vix < 15 ? "low fear, momentum plays favored." : levels.vix > 20 ? "high fear, reduce size." : "moderate volatility."}
              </p>
            </div>
          </div>
        </div>
